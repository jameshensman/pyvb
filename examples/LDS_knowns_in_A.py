...
if __name__=='__main__':
	plot=True
	#Learning of a linear dynamic system, where some of the elements in A are known. 
	# This is acheived by using the partial observation function of the Gaussian nodes in A.
	...
	true_A = np.array([[0,1],[-c/m,-k/m]])*dt + np.eye(2)
	
	true_C = np.random.randn(d,q)
	...
	#!!! observe known elements of A!!!
	As[0].observe(np.array([[1.],[np.nan]]))
	As[1].observe(np.array([[dt],[np.nan]]))
