...
class DiagonalGaussian(Gaussian):
	...
	def update(self):
		...
		#The messages have 4 parts: m1,m2,b,bbT, where b is the co-parent of the child (by multiplication)
		qprec = pprec + np.sum([e[0]*e[3] for e in child_messages],0) #that's elementwise multiplication, folks. NOT np.dot()
		self.qcov = np.linalg.inv(qprec)
		weighted_exs = np.dot(pprec,pmu) + np.sum([e[1]*e[2] for e in child_messages],0)
		self.qmu = np.dot(self.qcov,weighted_exs)
